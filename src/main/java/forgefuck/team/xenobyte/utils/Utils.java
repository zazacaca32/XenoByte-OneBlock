...
public class Utils {
    ...
    public double[] dcoords(Entity curEnt) {
        return new double[]{(double) curEnt.posX, (double) curEnt.posY, (double) curEnt.posZ};
    }
    ...
    public boolean isNpcDead(Entity e) {
        try {
            if (isCustom(e))
                return (boolean) e.getClass().getMethod("isKilled").invoke(e);
            return false;
        } catch (Exception ex) {
            return false;
        }
