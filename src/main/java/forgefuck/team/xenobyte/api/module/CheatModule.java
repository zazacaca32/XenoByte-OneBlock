...
import forgefuck.team.xenobyte.modules.MetaSell;
...
public abstract class CheatModule extends ModuleAbility implements Xeno {
    ...
    private EspDropSelect espDropSelector;
    private MetaSell meteSell;
    ...
    public void handlerInit(ModuleHandler handler) {
        if (moduleHandler() == null) {
            ...
            espDropSelector = (EspDropSelect) moduleHandler().getModuleByClass(EspDropSelect.class);
            giveSelector = (GiveSelect) moduleHandler().getModuleByClass(GiveSelect.class);

            meteSell = (MetaSell) moduleHandler().getModuleByClass(MetaSell.class);
            onHandlerInit();
        }
    }
    ...
    protected EspDropSelect espDropSelector() {
        return espDropSelector;
    }

    protected MetaSell getMeteSell() {
        return meteSell;
    }
